...
export function createLooper({ audio, syncStrudel } = {}) {
  ...
  const btnFreezeCfg = document.getElementById('btn-rig-freeze-cfg');
  const freezeCfgEl = document.getElementById('rig-freeze-cfg');
  ...
  // Freeze pad settings — persisted; applied before any freeze can start.
  ...
  function buildMiniUI() {
    ...
    // Order: tuner · effects+master · freeze · looper.
    row.append(tune, makeSep(), effects, makeSep(), fz, makeSep(), tr);
  ...
  function refreshMiniTuner() {
    ...
    miniTunerSq.textContent = (model.tunerOn && _strobe.voiced && _miniNote) ? _miniNote : '♪';
  }
  ...
  let _miniNote = '';                // mini square display: primary note + ↑/↓ direction
  ...
  // note, already carrying its ↑ sharp / ↓ flat direction — so the square is
  // a usable one-note tuner even in mini mode.
  ...
  function updateTunerMono(an) {
    ...
    _strobe.near    = near;
    // Mini square: note + which way it's off (↑ sharp / ↓ flat, ET-style).
    _miniNote = `${NOTE_NAMES[cls]}${octave}${inTune ? '' : (err > 0 ? '↑' : '↓')}`;
    refreshMiniTuner();
  }
  ...
  function drawStrobePoly(g, W, H, dpr) {
    ...
    // color, and which way it's off. dispCents (the calm readout value) drives
    // the arrow so it can't flicker at the in-tune boundary.
    if (prim) {
      const dir = prim.inTune ? '' : (prim.dispCents > 0 ? '↑' : '↓');
      setMini(true, prim.inTune, prim.near && !prim.inTune, prim.label + dir);
    } else {
      setMini(false, false, false, '');
    }
  }
  ...
  function initFreezeCfgUI() {
    ...
    const snap = (el, v) => {
      ...
      for (const o of el.options) if (Math.abs(parseFloat(o.value) - v) < Math.abs(parseFloat(best) - v)) best = o.value;
      el.value = best;
    };
    snap(freezeGrainEl, cfg.grainSec);
    snap(freezeReleaseEl, cfg.releaseSec);
    freezeLevelEl?.addEventListener('input', () => {
      looperAudio.setFreezeConfig({ level: parseFloat(freezeLevelEl.value) });
      saveFreezeCfg();
    });
