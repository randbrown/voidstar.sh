...
const EMMONS_COLORS = ['#f8d255', '#f5463c', '#5f8cb9', '#f8a537'];
// Sho-Bud classic red + the non-red suit ink options.
const SHOBUD_RED  = '#dc231e';
const SHOBUD_INK  = { white: '#f8f8f8', black: '#181818' };
...
// Sprite bake: shapes are drawn at SHAPE_R px radius into an SPR px square
// (padding for the 1.2× atom orbits + glow), then scaled at blit time.
const SPR     = 192;
const SHAPE_R = 60;
...
export default {
  ...
  async create(canvas, { ctx }) {
    ...
    // Rebuilt only when style/darkSuit change — never per frame.
    ...
    // Scratch canvases for the mask → tint → glow pipeline.
    const maskCanvas = makeLayer();
    const tintCanvas = makeLayer();
    ...
    // Suit sprite: union silhouette, tinted, baked glow. Outline style is a
    // true contour ring — dilate the mask (16 offset stamps) then punch the
    // mask back out, leaving a uniform-width band that follows the union
    // edge exactly (the Canvas2D stand-in for the Python's cv2.drawContours).
    function bakeSuit(suit, color, outline) {
      const spr = makeLayer();
    ...
    // Atom sprite: three orbit ellipses at 0/60/120° + nucleus, glow baked.
    // Electrons are live (they orbit), drawn per frame in render().
    function bakeAtom(color) {
      const spr = makeLayer();
      const c = spr.getContext('2d');
    ...
    const atomSprites = EMMONS_COLORS.map(bakeAtom);
    // White outlines used as the beat-shimmer edge pass over solid suits.
    const shimmerSprites = SUITS.map(s => bakeSuit(s, '#ffffff', true));
    ...
    function ensureSuitSprites(style, ink) {
      ...
      suitSprites = SUITS.map(s => {
        ...
        return bakeSuit(s, red ? SHOBUD_RED : SHOBUD_INK[ink], outline);
      });
    }
    ...
    return {
      ...
      dispose() {
        ...
        tintCanvas.width = tintCanvas.height = 0;
