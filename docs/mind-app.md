...
| Sketch | `sketch.js` | Blank-page drawing with the SAME annotate pipeline — a sketch is just a generated paper-colored PNG attachment (dark/light chooser, fixed hexes; `ocrStatus:'skipped'` since the base bytes stay blank) opened straight in the annotate view, so strokes stay editable and ride sync like any annotation. Entry points: "✎ sketch" in the editor attachment strip and on the home header (fresh note, page inline in the body), "New sketch" in the command palette, and the `#capture/sketch` deep link (manifest shortcut "new sketch") — a zero-tap external-launch target (e.g. an Android S Pen-removal automation) that skips the paper chooser (last-used paper) and lands straight on the canvas; the hash is replaced with the note's annotate route so back/reload can't mint twins. |
