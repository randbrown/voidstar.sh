// Procedural sample-pack generator — dependency-free.
//
// Renders one synthetic drum pack per genre (voidstar / lofi / tape / dub /
// jazz / metal / death / hiphop) to 16-bit mono WAV under
// `public/samples/<genre>/`, each with a Strudel-format `strudel.json`. These
// are the *sample* variant of every kit; the *synth* variants live in
// sequencer-voices.js. Both engines load these manifests:
//   - Strudel  → samples() (registered with a `<genre>_` prefix, so
//                s("bd sd").bank("metal") works without clobbering stock banks)
...
//
// Run:  node scripts/gen-samples.mjs   (or `npm run gen:samples`)
...
for (const genre of GENRES) {
  ...
  const manifest = { _base: `/samples/${genre.id}/` };
  let total = 0;
